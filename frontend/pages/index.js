...
export default class Index extends Component {
    ...
    this.state = {
      ...
      uploadId: '',
      fileName: '',
      backendUrl: 'http://localhost:5000/api/UploadFile'
    }
	}
  ...
  async startUpload(event) {
    try {
      ...
      let resp = await axios.post(`${this.state.backendUrl}/start-upload`,
       {   
          BucketName:"uploaddatazip",
          FileName: this.state.fileName,
          FileType: this.state.selectedFile.type
      })
      console.log(resp)
      let fileNamer = resp.data.jobId
      let uploadIdr = resp.data.uploadId
      this.setState({uploadId: uploadIdr })
      this.setState({fileName: fileNamer})
...
// ===============================================
  ...
  async uploadMultipartFile() {
    try {
      ...
      const FILE_CHUNK_SIZE = 100000000 // 100MB
      ...
      for (let index = 1; index < NUM_CHUNKS + 1; index++) {
        ...
        let getUploadUrlResp = await axios.post(`${this.state.backendUrl}/get-upload-url`, {
            BucketName:"uploaddatazip",
            JobId: this.state.fileName,
            PartNumber: index,
            UploadId: this.state.uploadId
        })

        console.log(getUploadUrlResp.data)
